...
#     options.append('AV_Obfuscator')
    ...
    options.append('All_AV_Obfuscator')
...
#         get_avs()
    ...
    for option in options:    
        ...
        count_what = 'Apps'
        if option == 'All_AV_Obfuscator' or option=='all':
            per_what_file = os.path.join(results_dir,'adam_detection_per_av_obfuscator.csv')
            qry = """
                select CONCAT(av,'-ADAM') as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from av_results_adam r                
                group by av, is_malicious, correct
            """    
            detection_rate(qry,per_what_file, option,count_what)         
        if option == 'AV' or option=='all':
            per_what_file = os.path.join(results_dir,'all_detection_per_av.csv')
            qry = """
                select av as per_what, is_malicious, correct, count(*) cnt
                from av_results_ADAM
                group by av, is_malicious, correct                
                """       
            detection_rate(qry,per_what_file, option,count_what)         
        ...
        if option == 'AV_Obfuscator' or option=='all':
            per_what_file = os.path.join(results_dir,'adam_detection_per_av_obfuscator.csv')
            qry = """
                select CONCAT(avs.av,'-ADAM') as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from av_results_adam r , av_products avs
                where r.av=avs.av
                group by avs.av, is_malicious, correct
            """    
            detection_rate(qry,per_what_file, option,count_what)         
    
        if option == 'AV_Obf_Strategy' or option=='all':
            per_what_file = os.path.join(results_dir,'adam_detection_per_av_obf_strategy.csv')
            qry = """
                select CONCAT(av,'-',obf_strategy) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from adam_cf_apps a, av_results_adam r
