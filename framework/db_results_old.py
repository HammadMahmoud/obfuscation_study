...
#         get_avs()
    ...
    for option in options:    
        ...
        if option == 'AV_Obfuscator' or option=='all':
            ...
            qry = """
                select CONCAT(avs.av,'-',obfuscator) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_benign r, av_products avs
                where a.scan_id = r.scan_id and r.av=avs.av and
                a.scan_id is not null and a.dataset = 'benign'
                group by avs.av,obfuscator, is_malicious, correct
                UNION ALL
                select CONCAT(avs.av,'-',obfuscator) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_all_malicious r, av_products avs
                where a.scan_id = r.scan_id and r.av=avs.av and
                a.scan_id is not null and a.dataset != 'benign'
                group by avs.av,obfuscator, is_malicious, correct;
            """    
        ...
        if option == 'AV_Obf_Strategy' or option=='all':
            ...
            qry = """
                select CONCAT(avs.av,'-',upper(obf_strategy)) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_benign r, av_products avs
                where a.scan_id = r.scan_id and r.av=avs.av and
                a.scan_id is not null and a.dataset = 'benign'
                group by avs.av,obf_strategy, is_malicious, correct
                UNION ALL
                select CONCAT(avs.av,'-',upper(obf_strategy)) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_all_malicious r, av_products avs
                where a.scan_id = r.scan_id  and r.av=avs.av and
        ...
        if option == 'AV_Original_Apps_Year' or option=='all':
            ...
            qry = """
                select CONCAT(avs.av,'-',app_year) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_benign r, av_products avs
                where a.scan_id = r.scan_id and obfuscator = '' and r.av=avs.av and
                a.scan_id is not null and a.dataset = 'benign'
                group by avs.av,app_year, is_malicious, correct
                UNION ALL
                select CONCAT(avs.av,'-',app_year) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_all_malicious r, av_products avs
                where a.scan_id = r.scan_id and obfuscator = '' and r.av=avs.av and
        ...
        if option == 'AV_All_Apps_Year' or option=='all':
            ...
            qry = """
                select CONCAT(avs.av,'-',upper(app_year)) as per_what, is_malicious, correct, count(distinct r.scan_id) cnt
                from apps a, av_results_benign r, av_products avs
                where a.scan_id = r.scan_id and r.av=avs.av and
                a.scan_id is not null and a.dataset = 'benign'
                group by avs.av,app_year, is_malicious, correct
